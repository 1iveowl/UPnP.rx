...
1. `SSDP.UPnP.PCL`: no `ConfigureAwait(false)` anywhere (0 of 26 awaits; SimpleHttpListener.Rx applies it consistently) — a UI-app consumer can deadlock/hop contexts unnecessarily. **Re-verified against shipped 8.0.0 (2026-07-26): still open** — the assembly carries zero `ConfiguredTaskAwaitable`/`ConfiguredValueTaskAwaitable` references, while `SimpleHttpListener.Rx` carries them. Non-breaking, so it can ship in any release.
2. `SSDP.UPnP.PCL`: `Device` sync-`Dispose` + documented "call `ByeByeAsync` first" — `IAsyncDisposable` candidate (already noted in the disposal model). **Re-verified against shipped 8.0.0 (2026-07-26): still open** — reflection shows `Device : IDevice, IDisposable` exposing both `ByeByeAsync` and `Dispose`, with no `IAsyncDisposable`; `ControlPoint`/`IControlPoint` are likewise `IDisposable`-only. **Breaking, so it belongs in 9.0.0 alongside candidates 7/8/9 or it waits for 10.0.0.**
