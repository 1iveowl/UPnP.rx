...
- **Phase 6 — CI detail:** ~~run `dotnet test` against the test csproj, not the slnx, once samples join the solution~~. **Reversed in 6.0.0 P2:** `dotnet test` on the `.slnx` handles non-test projects correctly, and naming one project by hand is how a sibling repo shipped 50 analyzer tests that CI never ran. Test the solution.
...
*(An earlier revision listed a tenth entry here that was ours, not upstream. It moved to [upnp-rx-v5.0-plan.md §12](upnp-rx-v5.0-plan.md) and shipped in 5.0.0 - this list is for gaps in the sibling libraries only, per the §3 rule.)*
