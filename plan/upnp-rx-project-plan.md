...
7. **Dependency addition (author-approved 2026-07-24): `SimpleHttpListener.Rx` as a direct reference, pinned 7.2.0 (bumped to 7.3.0 with the candidate #4 fix, alongside SSDP.UPnP.PCL 8.0.0).** The 4.0 eventing listener consumes it directly (`ToHttpListenerObservable`, `HttpSender`), so a transitive-only reference was a smell - and 7.2.0 carries the packet-information `LocalEndPoint` fix (upstream issue candidate #3 in §9, fixed by the author in the upstream repo), so the direct pin also floors consumers onto the corrected behavior. Amends the §5 v1 dependency lock. The wildcard-address defense in `LocalRoute` stays regardless (belt and braces, and correct for older runtime graphs).
...
1. `SSDP.UPnP.PCL`: no `ConfigureAwait(false)` anywhere (0 of 26 awaits; SimpleHttpListener.Rx applies it consistently) — a UI-app consumer can deadlock/hop contexts unnecessarily. **RESOLVED (SSDP.UPnP.PCL 9.0.0, 2026-07-26)** — 8.0.0 carried zero `ConfiguredTaskAwaitable` references; 9.0.0 carries them.
2. `SSDP.UPnP.PCL`: `Device` sync-`Dispose` + documented "call `ByeByeAsync` first" — `IAsyncDisposable` candidate (already noted in the disposal model). **RESOLVED (SSDP.UPnP.PCL 9.0.0, 2026-07-26)** — `Device`, `ControlPoint`, `IDevice` and `IControlPoint` all now implement `IAsyncDisposable`; `Device` gained a bounded `ByeByeTimeout`.
...
4. **RESOLVED (SimpleHttpListener.Rx 7.3.0 + SSDP.UPnP.PCL 8.0.0, 2026-07-25):** the shared RefCounted message streams did not survive dispose-then-immediately-resubscribe (observed 2026-07-25, dashboard rescan): when the subscriber count touches zero the socket teardown races the restart, and the canceled `TcpListener.AcceptTcpClientAsync` surfaces as `SocketException (89)` through `OnError` on the *new* subscription (SHL's accept loop treats only `OperationCanceledException` as a normal stop). Two upstream angles: SHL's accept/receive loops should treat cancellation-induced `SocketException` as completion, and the shared streams should tolerate restart. Downstream workaround (dashboard): overlap the old and new subscriptions so the count never reaches zero.
5. **RESOLVED (SSDP.UPnP.PCL 8.0.0, 2026-07-25 - the idiomatic flavor: `Start`/`IsStarted` removed, lazy start-on-first-subscription; `UpnpClient._startLock`/`EnsureStarted` deleted downstream):** `ControlPoint.Start` was neither idempotent nor thread-safe (unsynchronized `IsStarted`, throws on the second call) - the sole reason for `UpnpClient._startLock` downstream (2026-07-25 lock review). Fix flavors: minimal - an `Interlocked` gate making repeat calls no-ops; idiomatic - remove explicit `Start` and let the shared observables start lazily on first subscription (`Defer`), deleting the start-once concept from the stack. Either removes a downstream lock.
6. **RESOLVED (parsing in 9.0.0, modelling in 9.1.0; both verified against the shipped assemblies, 2026-07-27).** `SSDP.UPnP.PCL` 8.0.0: **`SsdpMessageParser.ParseMaxAge` returns 0 for valid compound `CACHE-CONTROL` headers.** It splits on `=` and requires exactly two parts, so `max-age=1800, must-revalidate` and `public, max-age=1800, s-maxage=600` both yield 0 (verified 2026-07-26 by calling the shipped 8.0.0 method directly); `no-cache, max-age=1800` survives only by luck, because that string happens to contain one `=`. Multi-directive Cache-Control is ordinary per RFC 9111, and 0 is not a neutral answer - it is also what an absent header produces, so "unknown" and "the device said zero" are one value. UPnP.Rx is insulated by accident rather than design: `RosterSource` maps a non-positive max-age onto `RosterExpiryFallback` (30 min) and `DescriptionCache` guards `MaxAge > TimeSpan.Zero`, so the device's true lifetime is silently replaced by our default instead of expiring it on the spot - but a consumer without those guards drops such devices immediately. Fix upstream: parse the directive list properly and return a nullable so absent stays absent. **Both halves have now shipped.** 9.0.0 fixed the directive parsing (`max-age=1800, must-revalidate` yields 1800); 9.1.0 added `TryParseMaxAge` returning `int?` and a nullable `MaxAge` on `Notify`/`MSearchResponse`, obsoleting `CacheControl` with a message naming the conflation. Five previously indistinguishable inputs now separate: a genuine `max-age=0` reports `TimeSpan.Zero` while absent, unparsable, negative and directive-less headers report null. Adopted downstream in 5.0.0 - `UpnpClient.ToMaxAge` deleted, `Announcement.MaxAge` reports what the device said, and the roster's fallback decision moved to where it belongs (a live device cannot have meant a zero deadline).
7. **RESOLVED in 9.0.0 (verified 2026-07-26 against the shipped assembly).** `SSDP.UPnP.PCL` 8.0.0: **absent-vs-zero conflation on `BOOTID`.** `Notify.BOOTID` / `MSearchResponse.BOOTID` are non-nullable `uint` filled by `ParseUIntOr(…, 0)`, while every neighbouring optional header - `CONFIGID`, `SEARCHPORT`, `NEXTBOOTID` - is nullable via `ParseNullable*`. BOOTID is mandatory for UDA 1.1+ devices but legitimately absent on UPnP 1.0 ones, and 0 is itself a legal value (UDA 2.0 §1.2.2 ranges it 0..2^31-1), so three states - absent, genuinely zero, unparsable - collapse into one. `ParseRfc1123Date` has the same shape, returning `DateTimeOffset.MinValue` as an absent-sentinel into a non-nullable `Date`. Making these nullable is breaking, hence a major.
8. **RESOLVED in 9.0.0 (verified 2026-07-26 against the shipped assembly).** `SSDP.UPnP.PCL` 8.0.0: **the UPnP 1.0 boot signature (`NLS`) is not parsed.** Pre-UDA-1.1 devices signal reboots with an `NLS` header carried under an RFC 2774 namespace prefix that the device negotiates via `OPT` (`OPT: "http://schemas.upnp.org/upnp/1/0/"; ns=01` → `01-NLS`). Both headers do survive into the extras dictionary - verified 2026-07-26 by driving the shipped HttpMachine 6.0.1 parser with a real device's response: header names are upper-cased on the way in (so lookups are case-safe, and the mixed-case headers real devices send are not a hazard), and neither `OPT` nor `01-NLS` sits in the standard-header filter set. So consumers *can* reach them; but resolving a dynamic namespace prefix is parser work no consumer should reimplement, and `Server.IsUpnp2` already tells the parser which dialect it is reading. A first-class `NLS` property would make reboot detection work for the entire UPnP 1.0 population. Downstream consequence and plan: [v4.1 plan §7d](upnp-rx-v4.1-plan.md) (4.3 shelf).
9. **RESOLVED in 9.0.0 (verified 2026-07-26 against the shipped assembly).** `SSDP.UPnP.PCL` 8.0.0: **`ParseDeviceInfo` misreports the UPnP version carried by `SERVER`/`USER-AGENT`.** Four defects, each verified 2026-07-26 by calling the shipped public method: (a) an absent header returns a `Server` claiming `UpnpMajorVersion` "2" / `UpnpMinorVersion` "0" - silence becomes the strongest possible version claim, and inconsistently so, since `IsUpnp2` stays false; (b) any value with fewer than three space-separated tokens fabricates "1.0", so a device sending exactly `UPnP/1.1` is reported as 1.0; (c) the second token is assumed to *be* the UPnP token without checking it begins with `UPnP/`, so `Windows NT/5.1, UPnP/1.0` reports 5.1, and the real Platinum renderer at hand - `SERVER: UPnP/1.0, DLNADOC/1.50 Platinum/1.0.5.13` - reports **1.50**, having read the DLNA doc version as the architecture version; (d) `IsUpnp2` is `major == "2"`, hence false for UDA 1.1 devices, which are not 1.0. UDA 2.0 defines the field (clauses 1.2.2, 1.3.3 and 3.2.2, identically) as product tokens in which "the second token represents the UPnP version and shall be UPnP/2.0"; the robust read is to find the token whose name is `UPnP` and report its version, and to report *nothing* when the header is absent or unparsable (house leniency: an unset optional field stays unset, never a fabricated value). Blocks the 4.3 version badge - see [v4.1 plan §7e](upnp-rx-v4.1-plan.md).

10. `UPnP.Rx` (ours, not upstream): **GENA subscribers do not watch discovery for boot-identity changes.** UDA 2.0 clause 4.1.1: "if the value of the BOOTID.UPNP.ORG is increased without a prior ssdp:update message with a matching NEXTBOOTID.UPNP.ORG field value, subscribers shall assume that their subscriptions have been cancelled". Nothing in `Eventing/` observes the discovery streams, so a rebooted device's dead subscription is only noticed when a renewal fails. 5.0.0 supplies the two missing prerequisites - `DeviceRebooted` distinguishes an unannounced boot change from an `ssdp:update`, and the roster now records the expected boot identity - so the remaining work is wiring `GenaSubscriptionSource` to resubscribe on `DeviceRebooted`. Not a regression; recorded 2026-07-27 by the 5.0.0 spec review and not covered by the clause-4 review. Candidate for 5.1.
